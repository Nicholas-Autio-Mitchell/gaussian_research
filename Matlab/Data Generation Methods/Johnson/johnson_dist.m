%% Johnson wrapper function
% This function uses the Johnson method of moments/quantiles to
% produce a random set of numbers with sample moments equal to those
% entered. It is possible to either specify desired moments or 'weights'
% for four quantiles, in order to shape the PD curve. 
% As the output is generated randomly, the sample moments may differ
% slightly from the desired moments given as inputs, especially the
% skewness and kurtosis.

function [sim_data, mom_summary, multi_dist] = johnson_dist(arg1, arg2, arg3, arg4, m, n)
...
%               returns generate
% mom_summary = a cell (representing a simple table) that displays the
...
[sim_data, dist_type] = johnsrnd(arg1, arg2, arg3, arg4, m, n);
...
% small sample sizes, i.e. 'AICc' (Burnham, Anderson, 2004, Multimodel 
% Inference: Understanding AIC and BIC in Model Selection)
informationCriterion = 'AIC';

% Perform the distribution comparisons, plotting the probability density
% functions of the data from the best four distributions, according to the
% given information criterion
multi_dist = allfitdist(sim_data, informationCriterion, 'PDF');

% Create the log-returns
log_ret_p = zeros(m, n);    % predefine the matrix dimensions
